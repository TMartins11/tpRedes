package br.unifal.redes.sender.fsm;
...
/**
 * Máquina de estados do Emissor no protocolo Go-Back-N.
 *
 * <p>Esta classe concentra <strong>toda</strong> a lógica de protocolo do
 * lado do Emissor — handshake, janela deslizante, retransmissão por
 * timeout — no mesmo papel que o comentário de integração em
 * {@code Receiver.java} reserva para a futura {@code GbnReceiverFsm} do lado
 * do Receptor. {@code Sender.java} (o bootstrap, análogo a {@code Receiver.java})
 * não implementa nada disso diretamente; apenas prepara a infraestrutura
 * (socket, blocos de arquivo, parâmetros da sessão) e a entrega a esta classe.
 *
...
public final class GbnSenderFsm {
     ...
     */
    public GbnSenderFsm(SenderSocketService socketService,
